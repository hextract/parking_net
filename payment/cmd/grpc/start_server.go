...
import (
	...
	"github.com/h4x4d/parking_net/payment/internal/grpc/handlers"
...
func StartServer(group *sync.WaitGroup) {
	...
	go func() {
		log.Printf("Starting Payment gRPC server on %s...", address)
...
func gracefulStop(server *grpc.Server) {
	...
	select {
	case <-done:
		log.Println("Payment gRPC server gracefully stopped.")
	case <-ctx.Done():
		log.Println("Timeout reached; forcing Payment gRPC server shutdown.")
		server.Stop()
	}
}
