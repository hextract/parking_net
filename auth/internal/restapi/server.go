...
// NewServer creates a new api parkings auth server but does not configure it
func NewServer(api *operations.ParkingsAuthAPI) *Server {
	s := new(Server)
...
// Server for the parkings auth API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/parkings-auth.sock"`
	...
	api          *operations.ParkingsAuthAPI
...
func (s *Server) SetAPI(api *operations.ParkingsAuthAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
